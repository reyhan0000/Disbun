            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @forelse($notifications as $notif)
                    <div class="p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors {{ $notif->is_read ? 'bg-white' : 'bg-blue-50' }}">
                        <div class="flex items-start gap-3">
                            ...
                            @endif
                            <div class="flex-1">
                                <a href="{{ route('petani.notifikasi.show', $notif) }}" class="block">
                ...
                @empty
                    <div class="p-8 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Anda akan menerima notifikasi tentang pengajuan Anda di sini.</p>
                    </div>
                @endforelse

                @if($unreadCount > 0)
                <div class="p-4 border-t border-gray-100 bg-gray-50">
                    <form action="{{ route('petani.notifikasi.markAllRead') }}" method="POST" class="text-center">
                        @csrf
